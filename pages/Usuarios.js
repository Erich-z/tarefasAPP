...
const windowHeight = Dimensions.get('window').height;
  ...
  const ItemList = ({tarefa})  =>{
    ...
    return(
    <TouchableOpacity onPress={()=> navigation.push('UsuarioDados', {tarefa:tarefa})}>
    <View style={[style.container, {backgroundColor: toggleCheckBox ? '#900' : '#EBF8A4'}]}>
    <View style={style.boxtext}>
    <Text style={{fontWeight: 'bold', color: '#000'}}>{tarefa.Nome}</Text>
    <Text style={{fontWeight: 'bold', color: '#000'}}>{tarefa.Email}</Text>
    </View>
    <CheckBox
    ...
    return(
     <SafeAreaView style={{height:windowHeight}}>
       ...
       <View style={{flexDirection: 'row', alignItems:'center', }}>
        <Text style={{color: '#000', fontWeight: 'bold', fontSize: 40, margin: 20, }}>Usuarios </Text>
        <TouchableOpacity style={{right: 10}} onPress={() => navigation.push('HomeScreen')}><Icon.List stroke="#000" fill="#000" width={32} height={32} /></TouchableOpacity>
